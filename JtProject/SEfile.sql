...
ALTER TABLE categories DROP PRIMARY KEY, ADD PRIMARY KEY (categoryid);
...
ALTER TABLE `products`
  ADD CONSTRAINT `products_ibfk_2` FOREIGN KEY (`categoryid`) REFERENCES `categories` (`categoryid`) ON DELETE CASCADE;
...
ALTER TABLE `users`
CHANGE COLUMN `role` `role` VARCHAR(250) NOT NULL DEFAULT 'ROLE_USERS';
